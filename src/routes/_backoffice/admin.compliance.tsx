...
function AdminCompliance() {
  ...
  return (
    <>
      ...
      <AdminCard>
        {(data ?? []).length === 0 ? (
          <p className="text-sm text-yo-txt-3 text-center py-8">Sin perfiles de riesgo en cola.</p>
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody>
              {(data ?? []).map((p: Record<string, unknown>) => (
                <tr key={p.id as string} className="border-b border-yo-border">
                  <td className="py-2 text-yo-txt-2 font-mono text-xs">{(p.user_id as string)?.slice(0, 8)}</td>
                  <td className="text-yo-txt font-semibold">{p.score as number}</td>
                  <td>
                    <span className={
                  ...
                  <td className="text-yo-txt-3 text-xs">{new Date(p.updated_at as string).toLocaleString("es-MX")}</td>
                </tr>
              ))}
            </tbody>
          </table>
