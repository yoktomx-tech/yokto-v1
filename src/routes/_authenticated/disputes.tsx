...
function OpenDisputeModal({ onClose }: { onClose: () => void }) {
  ...
  const [outcome, setOutcome] = useState("correction");
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-lg overflow-hidden rounded-[12px] bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-[#EBEBF0] px-5 py-4">
          <div>
            <h3 className="text-base font-semibold text-[#18181B]">Abrir disputa</h3>
            <p className="text-xs text-[#71717A]">Paso {step} de 3</p>
          </div>
          <button onClick={onClose} className="rounded-[6px] p-1 hover:bg-[#F4F4F7]"><X className="h-4 w-4 text-[#52525B]" /></button>
        </div>

        <div className="space-y-4 p-5">
          <div className="rounded-[8px] bg-[#F0F9FF] px-3 py-2 text-[11px] text-[#075985]">
            <Info className="mr-1 inline h-3 w-3" />
            La plataforma actúa como tercero neutral. La resolución se basará en evidencia verificable.
          </div>

          {step === 1 && (
            <>
              <Field label="Operación">
                <input value={tx} onChange={(e) => setTx(e.target.value)} placeholder="OP-2091" className={inputCls} />
              </Field>
              <Field label="Hito relacionado">
                <input value={milestone} onChange={(e) => setMilestone(e.target.value)} placeholder="Hito 2 · Entrega final" className={inputCls} />
              </Field>
              <Field label="Monto afectado (MXN)">
                <input value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00" className={cn(inputCls, "font-mono")} />
              </Field>
            </>
          )}
          ...
          {step === 2 && (
            <>
              <Field label="Motivo">
                <select value={reason} onChange={(e) => setReason(e.target.value as DisputeReason)} className={inputCls}>
                  <option value="">Selecciona un motivo</option>
                  {Object.entries(REASON_LABEL).map(([k, l]) => <option key={k} value={k}>{l}</option>)}
                </select>
              </Field>
              <Field label="Descripción">
                <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={4} placeholder="Explica la situación con evidencia verificable..." className={inputCls} />
                <p className="mt-1 text-[10px] text-[#A1A1AA]">Mínimo 40 caracteres. Evita lenguaje acusatorio.</p>
              </Field>
              <Field label="Resultado solicitado">
                <select value={outcome} onChange={(e) => setOutcome(e.target.value)} className={inputCls}>
                  <option value="correction">Corrección</option>
                  <option value="partial">Resolución parcial</option>
                  <option value="refund">Devolución total</option>
                  <option value="release">Liberación con condiciones</option>
                </select>
              </Field>
            </>
          )}
          ...
          {step === 3 && (
            <>
              <Field label="Adjuntar evidencia">
                <div className="rounded-[10px] border-2 border-dashed border-[#EBEBF0] bg-[#F4F4F7] px-4 py-6 text-center text-xs text-[#71717A]">
                  Arrastra archivos o haz clic para seleccionar
                </div>
              </Field>
              <label className="flex items-start gap-2 rounded-[8px] bg-[#F4F4F7] p-3 text-xs text-[#52525B]">
                <input type="checkbox" checked={confirm} onChange={(e) => setConfirm(e.target.checked)} className="mt-0.5" />
                Confirmo que la información presentada es correcta y entiendo que quedará registrada en el expediente auditado de la operación.
              </label>
            </>
          )}
        </div>

        <div className="flex items-center justify-between border-t border-[#EBEBF0] bg-[#F4F4F7] px-5 py-3">
          <button onClick={step === 1 ? onClose : () => setStep(step - 1)} className="text-sm text-[#52525B] hover:text-[#18181B]">
            {step === 1 ? "Cancelar" : "← Atrás"}
          </button>
          <button
            onClick={step === 3 ? onClose : () => setStep(step + 1)}
            disabled={step === 3 && !confirm}
            className="rounded-[8px] bg-[#4F46E5] px-4 py-2 text-sm font-medium text-white hover:bg-[#4338CA] disabled:opacity-50"
          >
            {step === 3 ? "Abrir disputa" : "Continuar →"}
          </button>
        </div>
      </div>
    </div>
...
const inputCls = "w-full rounded-[8px] border border-[#EBEBF0] bg-white px-3 py-2 text-sm text-[#18181B] placeholder:text-[#A1A1AA] focus:border-[#4F46E5] focus:outline-none focus:ring-2 focus:ring-[#4F46E5]/15";
